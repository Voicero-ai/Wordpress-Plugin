...
const VoiceroCore = {
  ...
  init: function () {
    ...
    if (typeof aiWebsiteConfig !== 'undefined') {
      ...
      console.warn('Voicero Core Script: No config found (aiWebsiteConfig is undefined)');
    }
  ...
  initializeApiConnection: function() {
    ...
    if (window.aiWebsiteConfig && window.aiWebsiteConfig.accessKey) {
      console.log('Voicero Core Script: Found access key, checking API connection');
  ...
  createButton: function () {
    ...
    console.log('Voicero Core Script: Should auto-open:', shouldAutoOpen);
    ...
    if (!container) {
      ...
      document.body.insertAdjacentHTML("beforeend", `<div id="voicero-app-container"></div>`);
      container = document.getElementById('voicero-app-container');
    }
    
    if (container) {
      console.log('Voicero Core Script: Container found, adding button interface');
      ...
      if (buttonContainer) {
        ...
        console.log('Voicero Core Script: Button interface created successfully');
      } else {
        console.error('Voicero Core Script: Failed to create button container');
      }
    } else {
      console.error('Voicero Core Script: Could not find or create container');
    }
  },
  ...
  createTextChatInterface: function () {
    ...
    if (container) {
      ...
      console.error('Voicero Core Script: Could not create text chat interface');
    }
  },
  ...
  createVoiceChatInterface: function () {
    ...
    if (container) {
      ...
      console.log('Voicero Core Script: Voice chat interface container added');
    } else {
      console.error('Voicero Core Script: Could not create voice chat interface');
    }
  },
  ...
  checkApiConnection: function (accessKey) {
    console.log('Voicero Core Script: Starting API connection check with key:', accessKey.substring(0, 10) + '...');
    ...
    const tryNextUrl = () => {
      ...
      fetch(apiUrl, {
        ...
        .then((response) => {
          console.log('Voicero Core Script: API response status:', response.status);
          if (!response.ok) {
        ...
        .then((data) => {
          ...
          console.log('Voicero Core Script: Is website active?', data.website?.active);
          ...
          if (data.website && data.website.active === true) {
            console.log('Voicero Core Script: Website is active, creating interface');
            ...
            if (this.appState.isOpen && this.appState.activeInterface) {
              console.log('Voicero Core Script: Auto-opening interface:', this.appState.activeInterface);
            ...
            console.warn('Voicero Core Script: Website is not active in API response');
          }
        })
        .catch((error) => {
          console.error(`Voicero Core Script: API error with ${currentUrl}:`, error);
